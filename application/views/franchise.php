<body onload="get_franchise_details()">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button form="add_data_form" class="btn btn-success" id="btn_add_data">Add New Franchise</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById("franchise").classList.add('active');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(franchise, function(i) {
                        ...
                        str += `<tr>
                                <td>${i+1}</td>
                                <td>${franchise[i].name}</td>
                                <td>${franchise[i].email}</td>
                                <td>${franchise[i].mobile}</td>
                           
                                <td><label class="switch">
                                <input type="checkbox"  ${franchise_status} onclick="status(this,'${franchise[i].uid}')">
                                ...
                                <button class="hdrbtn mx-2 editUser" data-toggle="modal" id=" editbtn"  data-target="#edtView1"  onclick="edit_franchise('${franchise[i].uid}' , '${franchise[i].name}' , '${franchise[i].email}' , '${franchise[i].mobile}')">
                                ...
                                <button class="hdrbtn mx-2 delete_franchise" data-toggle="modal" data="${franchise[i].uid}" data-target="#deltmodl" >
        ...
        function status(state, uid) {
            if (state.checked == true) {
                $.ajax({
                    url: "<?= base_url('Admin/active_franchise') ?>",
                    type: "post",
                    data: {
                        "id": uid
                    },
                ...
                console.log('false');
                $.ajax({
                    url: "<?= base_url('Admin/deactive_franchise') ?>",
                    type: "post",
                    data: {
                        "id": uid
                    },
        ...
        $('#btn_update_data').on('click', function() {
            ...
            if (flag == 0) {
                $.ajax({
                    url: "<?= base_url('Admin/update_franchise') ?>",
                    type: "post",
                    data: {
                        "id": id,
                        "name": name,
        ...
        $('#btn_add_data').on('submit', function() {
            ...
            if (flag == 0) {
                $.ajax({
                    url: "<?= base_url('Admin/add_franchise') ?>",
                    type: "post",
                    data: {
                        "name": name,
                        "email": email,
        ...
        $('#table_details').on('click', '.delete_franchise', function() {
            ...
            $('#btn_delete_data').click(function() {
                $.ajax({
                    type: "post",
                    url: "<?= base_url('Admin/delete_franchise') ?>",
                    data: {
                        "id": id
                    },
                    async: false,
