<body onload="get_sarathi_details()">
      ...
      <div class="row align-items-center mb-4">
        ...
        <div class="col-md-4">
          <div class="d-flex align-items-center justify-content-end">
            <!-- <select class="form-control bgdarkred btnround" style="min-width: 150px">
        ...
        <div class="modal-body">
          <form id="update_form">
            <div class="row">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <input class="form-control" type="number" value="9876-543-210" placeholder="Your Number" id="edit_number">
                </div>
              </div>
  ...
  <script>
    ...
    function get_sarathi_details() {
      $.ajax({
        type: "post",
        url: "<?= base_url('Admin/get_sarathi') ?>",
        success: function(data) {

          let sarathi = JSON.parse(data);
          let str = '';
          let count = 1;
          let user_status = "";
          ...
          for (let i = 0; i < sarathi.length; i++) {
            ...
            str += `<tr>
                        <td>${count}</td>
                        <td><a href="<?=base_url("Admin/sarathi_details/")?>${sarathi[i].uid}">${sarathi[i].name}</a></td>
                        <td>${sarathi[i].email}</td>
                        <td>${sarathi[i].mobile}</td>
                     
                        <td><label class="switch">
                        <input type="checkbox"  ${user_status} id = "check-status" data ="${sarathi[i].uid}" onclick="status(this, '${sarathi[i].uid}')">
                        ...
                        <button class="hdrbtn mx-2 edit_user" data-toggle="modal" id=" editbtn"  data-target="#edtView1"  onclick="edit_sarathi('${sarathi[i].uid}' , '${sarathi[i].name}' , '${sarathi[i].email}' , '${sarathi[i].mobile}')">
                        ...
                        <button class="hdrbtn mx-2 delete_user" data-toggle="modal" data="${sarathi[i].uid}" data-target="#deltmodl" >
